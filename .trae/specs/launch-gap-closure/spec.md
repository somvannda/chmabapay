...
| ID | Sev | Gap | Evidence | Fix |
| --- | --- | --- | --- | --- |
| G-10 | S1 | Four endpoints require **no authentication** while the docs present a single Bearer surface: `POST /api/v1/khqr/from-link`, `/probe-aba-status`, `/payway/checkout`, `/payway/status`. Verified live: `POST /api/v1/khqr/from-link` with an empty body returns **422, not 401**. `payway/checkout` spends a real ABA session. | `src/chmabapay/routers/khqr.py:112,281,327,371` | Add the standard hybrid auth dependency to all four. |
...
| G-12 | S2 | The synthetic `/api/v1/webhooks/{id}/test` payload omits `financial` — the one field the docs tell integrators to branch on — plus `data.merchant`, `paid_at`, `expires_at`. | `src/chmabapay/routers/webhooks.py:88-111` vs `docs/api.md:198-200` | Build the test payload from the same builder as a real delivery. |
| G-13 | S2 | `GET /api/v1/payments/{id}` is documented as returning `checkout_url`; the router hardcodes `None`. `docs/api.md` correctly says it is not returned, so the docs page contradicts the prose doc. | `src/chmabapay/routers/payments.py:410` vs `web/landing/app/api/docs/page.tsx:60` | Correct the docs page. |
...
| G-16 | S2 | 9 Bakong Ledger Lookup endpoints are listed for customers while the same group's own text says they answer `503 bakong_not_configured` in production. Meanwhile the two transaction endpoints that **do** work without Bakong creds (`GET /api/v1/transactions/check-status/{id}`, `POST /api/v1/transactions/verify-payment/{id}`) are undocumented. | `page.tsx:66-80`; `routers/transactions.py:369-371,420-472` | Mark the dead group as unavailable; document the two live endpoints. |
| G-17 | S2 | The live `openapi.json` is unusable as a contract: no `components.securitySchemes`, every `authorization` param `required=false`, and no 201/402/403/404/409/410/503 responses on any operation. It also publicly enumerates all 9 `/api/v1/admin/*` paths. | live `openapi.json`; `routers/admin.py:19` | Add security schemes + declared error responses; `include_in_schema=False` on the admin router. |
...
| ID | Sev | Gap | Evidence | Fix |
| --- | --- | --- | --- | --- |
| G-21 | S1 | Terms acceptance is never enforced. `POST /api/v1/me/terms` works, but **no portal code calls it** and nothing gates on `terms_accepted_at`. A merchant can transact forever without accepting. | `src/chmabapay/routers/account.py:117-153`; no caller anywhere in `web/` | Add a blocking acceptance screen in the post-OAuth flow; enforce server-side so API keys cannot bypass. |
...
| G-23 | S1 | Even with an invoice, `GET /api/v1/billing/invoices/{id}/khqr` raises `500 platform_hq_store_not_configured`: `CHMABAPAY_HQ_STORE_ID` and `CHMABAPAY_HQ_PAYWAY_LINK` are **empty in `deploy/.env`** (verified), so no HQ store is ever seeded. The customer sees the raw code in a toast. | `routers/billing.py:199-234`; `routers/auth.py:245-283`; prod env | Decision D2. |
| G-24 | S1 | `POST /api/v1/billing/change-plan` collects no payment and applies no proration, and is one click from the dashboard → Pro ($59.99/mo, 50 stores, 1M payments) for $0. | `routers/billing.py:140-196`; `web/landing/app/dashboard/billing/page.tsx:396-457` | Decision D2. |
...
| G-31 | S2 | No account deletion / erasure path (`DELETE /api/v1/me` absent) and no password rotation. `PATCH /api/v1/me` changes email with no verification. | `routers/account.py:75-96` | Add erasure + password change; require re-verification for email change. Decision D4. |
...
| G-41 | S1 | Payment disputes cannot be resolved. No `GET /api/v1/admin/payments/{id}`, so an operator cannot see `bakong_ref` or `gateway_status_raw` — the two fields that answer "did money move?". No force-reconcile, no mark-paid, no re-deliver. | `routers/admin.py:703-815`; `routers/transactions.py:439-448` | Admin payment detail + reconcile + re-deliver + audit-logged mark-paid. |
| G-42 | S2 | The "password-only console" is enforced **only in React**: `session_auth_method` is referenced solely by `GET /api/v1/me`, so an admin's Google session can call every `/api/v1/admin/*` endpoint directly. | `routers/account.py:72` (only caller); `routers/admin.py:33-51` | Enforce `amr == "password"` inside `get_hybrid_admin_context`. |
...
- Edge authorisation on `/api/v1/admin/*` (401 unauthenticated, 401 with a bare `ck_` key, `is_platform_admin` required on all 12 routes, account-active enforced on both auth paths).
...
3. `POST /api/v1/khqr/from-link` and the other three KHQR routes return 401 without a
