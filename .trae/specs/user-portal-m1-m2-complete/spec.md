...
## Goals
1. Session auth UX end-to-end: no 404 or blank-screen flows. Logged-in users always land on `/dashboard` with session JWT cookie verified by `/api/v1/me`.
...
5. Session lifecycle: `/auth/signout` clears cookie and lands on `/` landing page; on cookie expiry, any page-wide `/api/v1/me` 401 → redirect to login with `?next=...` return path.
...
2. KYC document upload UI (file picker / preview / S3 storage) — M2 KYC only; today's Settings/KYC form accepts file URL strings only (matching current `/api/v1/kyc` POST schema).
...
- Existing state (2026-09-11):
  - Backend routers live at `src/chmabapay/routers/`: [auth.py](file:///e:/Development/chmabapay/src/chmabapay/routers/auth.py#L1-L495) (Google OAuth, session cookies, `/auth/signout`), [account.py](file:///e:/Development/chmabapay/src/chmabapay/routers/account.py#L1-L160) (`/api/v1/me` GET/PATCH, `/api/v1/kyc` POST), [stores.py](file:///e:/Development/chmabapay/src/chmabapay/routers/stores.py#L1-L120), `payments.py`, `transactions.py`, `keys.py`, `webhooks.py`, [billing.py](file:///e:/Development/chmabapay/src/chmabapay/routers/billing.py#L1-L200) (plans list, change-plan, subscription), `platform.py`, `reports.py`, `admin.py`, `checkout.py`, `khqr.py`, `dev.py`.
  - Next app routes today: `/`, `/api/docs`, `/dashboard` new. Rewrites set in [next.config.js lines 7-61](file:///e:/Development/chmabapay/web/landing/next.config.js#L7-L61) proxy `/api/v1/*`, `/auth/*`, `/user/google/auth/*`, `/pay/*`, `/openapi.json`, `/health` straight to backend. No CORS issues. No separate origins.
  ...
  - BRD §5 exit criteria for Sokha demo: sign-in with Google → paste ABA link → create store ACTIVE → make test payment via `/api/v1/payments` → trigger `/_dev/payments/{id}/pay` → webhook delivered signed to sink. We need all of these to also be clickable UI steps in the portal (not just cURL).
...
- **FR-Page-2 Stores index /dashboard/stores**:
  ...
  - Top CTA "+ Create store" opens 2-step wizard (step1: name + ABA payway paste OR step2: Bakong ID paste OR bank dropdown + account). Submit → POST `/api/v1/stores` with session cookie auth → backend enforce max_stores plan gate via plan/subscription lookup → on 400 detail "Upgrade to Growth plan to add more than N stores" → render inline alert; on 201 → toast success & redirect to detail.
  - Store detail `/dashboard/stores/[id]`: store fields, current link info (mask bakong/bank numbers sensibly), recent payments for store tab (GET /api/v1/payments?store_id=id).
- **FR-Page-3 Payments index /dashboard/payments**:
  ...
  - CTA "+ Create payment" opens wizard: amount, reference_id, store dropdown, metadata k/v → POST /api/v1/payments.
...
- **FR-Page-6 Billing & Plans /dashboard/billing**:
  ...
  - CTA per plan: "Upgrade" → POST `/api/v1/billing/plan-change` body `{ plan_code: "growth" }`. On 201 show success toast + subscription row updated. If plan downgrade is blocked by backend → render alert.
...
- **FR-Page-8 Sub-Merchants /dashboard/sub-merchants** (only if allow_saas_sub_merchants):
  ...
  - CTA "+ Add Sub-merchant" → wizard: display_name + external_id + aba_link_or_bakong_id → POST /api/v1/platform/sub-merchants (session allowed for cookie).
  - M1 skeleton: read-only view list if platform endpoint works; empty state otherwise with banner "Coming soon, use POST /api/v1/platform/sub-merchants via API today".
- **FR-Page-9 Reports index /dashboard/reports** (hidden Starter):
  - M1 placeholder page with CSV export buttons "Payments (30d).csv", "Stores.csv". Button click → fetch `/api/v1/reports/...` (existing reports router) and trigger browser download via hidden anchor.
...
- **FR-BE-2**: Cookie auth for `/api/v1/stores` POST/GET/PUT must correctly call `_enforce_max_stores` the same way Bearer does today.
- **FR-BE-3**: Cookie auth for `/api/v1/keys` create/rotate/revoke correctly enforces Individual = account_scope disabled.
- **FR-BE-4**: `/api/v1/payments` list + detail accepts session cookie with same filter/store scoping rules as Bearer.
...
- **NFR-3 Session refresh tolerance**: Frontend `/api/v1/me` 401 → redirect and do NOT leave the page half-rendered with sensitive stale state.
- **NFR-4 Build performance**: `next build` must exit 0 after adding all routes. First-load JS per route <= 100kB.
- **NFR-5 Plan gate consistency**: All lock badges, disabled buttons, hidden nav items are derived server-side from `/api/v1/me` fields + subscription state from `/api/v1/billing/subscription`. Do NOT hardcode plan logic on client beyond `profile.allow_saas_sub_merchants`, etc.
...
- [x] **Q2**: On dashboard overview metric cards today, real aggregate endpoints (sum paid today) — backend has no route for totals yet. Options: (a) add quick `/api/v1/metrics/overview` JSON endpoint; (b) derive stats by aggregating the existing payments list results client-side with a `limit=200` fetch; (c) show 0 placeholder until backend builds reports later. Preference? Default assumption is (b) — client-side aggregation limited to last 200 payments so M1 works without new routes. → resolved: neither (b) nor (c) shipped — `web/landing/app/dashboard/page.tsx` reads server-side aggregates from `GET /api/v1/reports/payments.json` (`summary.total_matching_paid_amount_cents`), implemented in `src/chmabapay/routers/reports.py`.
...
- **Then**: 302 callback sets cookie `chmabapay_session` → redirects to `/dashboard` → dashboard overview loads, fetches `/api/v1/me` 200, hero welcome shows account name, plan card shows current plan, sidebar has Overview/Stores/Payments/Keys/Webhooks/Billing/Reports[hidden]/Settings/Sign out
- **Pass Condition**: Every check above observed. Cookie exists (inspect Application tab) and httpOnly=true.
- **Evidence**: Browser DevTools screenshot of (a) cookie tab, (b) `/dashboard` full rendered page, (c) `/api/v1/me` network 200 payload.
...
- **Then**: Backend POST `/api/v1/stores` receives cookie, creates row in Postgres `stores` table, returns 201 StoreOut. Frontend redirects to `/dashboard/stores/[id]`. Table row shows new store.
...
- **Then**: 201 response with qr_string + checkout_url. Payments list shows the new row pending. Navigate to detail page, img src `checkout_url/qr.png` loads. Click dev "mark paid" (admin only OR allow via session for all in dev mode) → backend marks it paid. /api/v1/me session cookie auth for payment GET detail still works.
...
- **Then**: Refresh `/api/v1/me` → account_type=business, allow_account_scope_keys=true. Return to `/dashboard/keys` → Create key wizard shows Account scope tab enabled. Sidebar Sub-Merchants nav item is still Locked. Upgrade Scale → Sub-Merchants unlocks.
- **Pass Condition**: Every flag change observed from /api/v1/me response & UI reflects it.
- **Evidence**: Network /api/v1/me diff before/after + sidebar lock state.
...
- **Then**: /api/v1/kyc 200; reload /api/v1/me → kyc_status=submitted (or reviewing depending on backend behavior, per current code). Overview hero shows yellow KYC reviewing banner.
...
- **Then**: 303 redirect → landing `/`. Cookie `chmabapay_session` is gone or empty. Navigate manually /dashboard → /api/v1/me 401 → redirect login.
...
- **When**: View sidebar
- **Then**: Reports nav item is not rendered. Upgrade to Growth → refresh profile → Reports nav item appears. Click Reports → Payments CSV export button trigger downloads via /api/v1/reports route (or fallback empty CSV download M1 if reports router placeholder).
