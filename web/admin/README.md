...
and `GET /api/v1/me` reports `auth_method` so a Google session is rejected even if it
...
Google session cannot reach `/api/v1/admin/*` with curl either.
...
Every other page calls `GET /api/v1/me` on mount; signed-out visitors are redirected to
...
All calls go to `/api/v1/admin/*` and require an **admin session cookie** carrying
...
Since the console now sets the platform's own collection link, `PUT /api/v1/admin/hq-store/link`
...
Both it and `GET /api/v1/admin/hq-store` are gated the same way. The write is audit-logged
...
`PUT /api/v1/admin/stores/{public_id}/internal` flips that flag on any store by hand
...
- `POST /api/v1/admin/accounts/{account_id}/keys` and `POST /api/v1/admin/keys/{key_id}/rotate` share
...
- `GET /api/v1/admin/health` — the database probe, the worker transport, the expected queues and
  each one's last drain, with no connection strings.
- `GET /api/v1/admin/audit-logs/export?format=csv|json` — the *same* filters as the list route
  ...
  looking complete.
- `GET /api/v1/admin/accounts?q=` widened from email-and-name to store public id and key prefix.
